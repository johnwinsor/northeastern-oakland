...
def main():
    ...
    lastPOL = analyticsJson[0]['POL']
    analyticsDataLength = len(analyticsJson)
    print(f"Found {analyticsDataLength} records in Analytics report")
    newBooks = []
    missingCovers = []
    counter = 1
    hits = 0
    ...
    with open('../api/static/newbooks.json', 'r') as seenBooksJson:
        ...
        for book in analyticsJson:
            ...
            mmsId = book['mmsId']
            logging.info(mmsId)
            if any(d.get('mmsId') == mmsId for d in seenBooks):
            ...
            if book['SortDate'] == '0000-00-00':
                logging.info(book['Title'])
                logging.info("Book not received")
                nr += 1
                continue
            try:
                isbn13 = book['isbn13']
                    ...
                    if 'imageLinks' in googleItem['volumeInfo']:
                        ...
                        if googleCover:
                            ...
                            if openLibraryCover:
                                ...
                                missingCovers.append(missingbook)
                                misses += 1
                                continue      
                    else:
        ...
        with open('newFoundBooks.json', "w") as nb:
            json.dump(sortedNewBooks, nb, indent=4)
        
        with open('missingCovers.json', "r") as mbr:
            file_data = json.load(mbr)
            for new in missingCovers:
                file_data.append(new)
        
        with open('missingCovers.json', "w") as mbw:
            json.dump(file_data, mbw, indent=4)
        
        print("------------------------------------")
        print(f"Found {dataLength} records in existing dataset")
        print(f"Attempted to match {analyticsDataLength} new titles from Analytics report with enhanced metadata.")
        print(f"Found {dupes} titles in existing dataset")
        print(f"Matched {sortedNewBooksCount} titles with usable covers. Adding to Dataset and writing to newbooks.json.")
        print(f"New Dataset size: {sortedSeenBooksCount}")
        print(f"{nr} titles not yet received and skipped")
        print(f"Unable to match {misses} titles with usable covers")
        
        logging.info("------------------------------------")
        logging.info(f"Found {dataLength} records in existing dataset")
        logging.info(f"Attempted to match {analyticsDataLength} new titles from Analytics report with enhanced metadata.")
    ...
    source = currentPath / f"newbooks.json"
    ...
    print("DONE")
    print(f"Last POL Processed: {lastPOL}. Writing to lastRecord.txt")
